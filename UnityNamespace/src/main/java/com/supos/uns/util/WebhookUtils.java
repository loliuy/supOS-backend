...
public class WebhookUtils {
    ...
    public static List<WebhookDataDTO> transferDto(Collection<CreateTopicDto> unsList) {
        List<WebhookDataDTO> wdds = new ArrayList<>();
        for (CreateTopicDto uns : unsList) {
            // 只推送实例数据
            if (uns.getPathType() == 2 && uns.getDataType() != Constants.CITING_TYPE) {
                ...
                Map<String, Object> protocol = uns.getProtocol();
                if (protocol != null) {
                    wdd.setConfig(JsonUtil.toJson(protocol));
                }
